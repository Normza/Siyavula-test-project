generated_table = {
  generate: function() {
    generate_from = '0';
    generate_for = 'unset';
    tocparent = document.getElementById('generated-table');
    if (tocparent) {
      // there is a div class="generated-table" in the document
    ...
    if (generate_from == 0) {
      first_header_found = generated_table.findFirstHeader(top_node);
    ...
    a = document.createElement('a');
    a.id = 'generated_table_d_toggle';
    a.appendChild(document.createTextNode(toggle_initially));
    p.appendChild(a);
    a.onclick = generated_table.wrapOpenClose(a,cur_list_el);
    a.href = '#';
    tocparent.appendChild(p);
    ...
    for (var i=0; i<headings.length; i++) {
      ...
      // create a link to this heading, and add it to the Table
      li = document.createElement('li');
      a = document.createElement('a');
      a.href = '#' + this_head_el.id;
      a.appendChild(document.createTextNode(generated_table.innerText(this_head_el)));
      li.appendChild(a);
    ...
    // go through the Table and find all LIs that are "empty", i.e., contain
  ...
  wrapOpenClose: function(a, cur_list_el) {
    ...
    return function(e) {
      ...
      if (cur_list_el.style.display == "block") {
        generated_table.createCookie("generated_table_display","open",21);
      } else {
        generated_table.createCookie("generated_table_display","closed",21);
      }
      if (window.event) {
  ...
  findFirstHeader: function(node) {
    ...
    if (nn.match(/^h[1-6]$/)) {
      ...
      for (var i=0; i<node.childNodes.length; i++) {
        var subvalue = generated_table.findFirstHeader(node.childNodes[i]);
